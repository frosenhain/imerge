...
type EventItem = { id: number; date: string; time: string; name: string; format: string; space: string; status: "Confirmado" | "Últimos cupos" };
// AGENDA MOCK EDITABLE: agregar, quitar o cambiar estos objetos actualiza el calendario.
const events: EventItem[] = [
  { id: 1, date: "2026-09-04", time: "08:30", name: "Cumbre de innovación corporativa", format: "Conferencia", space: "Sala Inmersiva", status: "Confirmado" },
  { id: 2, date: "2026-09-10", time: "18:00", name: "Noche de networking ejecutivo", format: "Networking", space: "Terrace Lounge", status: "Últimos cupos" },
  { id: 3, date: "2026-09-17", time: "09:00", name: "Academia de liderazgo", format: "Capacitación", space: "Aulas Flex", status: "Confirmado" },
  { id: 4, date: "2026-09-24", time: "19:00", name: "Lanzamiento de marca", format: "Experiencia", space: "Sala Inmersiva", status: "Confirmado" },
];
const spaces = [
...
export default function Home() {
  ...
  const inquire = (event?: EventItem) => { if (event) setSelected(event); setMessage(""); setModal(true); };
  ...
  return <main>
    ...
    <section className="agenda section" id="agenda"><div className="heading"><div><p className="eyebrow">PRÓXIMAMENTE EN IMERGE</p><h2>La agenda<br/><em>se mueve aquí.</em></h2></div><button className="button dark" onClick={() => inquire()}>Proponer mi evento →</button></div><div className="agenda-grid"><div><div className="filters">{formats.map((format) => <button className={filter === format ? "selected" : ""} onClick={() => setFilter(format)} key={format}>{format}</button>)}</div><div className="event-list">{listed.map((item) => <button className={`event ${selected.id === item.id ? "active" : ""}`} onClick={() => setSelected(item)} key={item.id}><time><b>{new Date(`${item.date}T12:00:00`).getDate()}</b><span>{dateText(item.date).split(" ")[1]}</span></time><span><strong>{item.name}</strong><small>{item.time} · {item.space}</small></span><i className={item.status === "Últimos cupos" ? "limited" : ""}>{item.status}</i><b>↗</b></button>)}{!listed.length && <p className="empty">No hay eventos para este filtro aún.</p>}</div><p className="note">Agenda de demostración editable en <code>app/page.tsx</code>. No representa disponibilidad real.</p></div><aside><p className="eyebrow">EVENTO DESTACADO</p><span>{dateText(selected.date, true)} · {selected.time}</span><h3>{selected.name}</h3><dl><div><dt>Formato</dt><dd>{selected.format}</dd></div><div><dt>Espacio</dt><dd>{selected.space}</dd></div></dl><button className="button lime" onClick={() => inquire(selected)}>Consultar fecha →</button></aside></div></section>
