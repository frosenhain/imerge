...
export default function Home() {
  ...
  const scores = useMemo(() => dimensions.flatMap((dimension) => { const values = filtered.map((person) => person.scores[dimension.id]).filter(Number.isFinite); return values.length ? [{ dimension, value: mean(values) }] : []; }), [filtered]);
  ...
  return <main>
    ...
    <section className="content"><header><div><p className="eyebrow">MADUREZ Y ADOPCIÓN DE AI</p><h1>{mode === "class" ? "Un recorrido para conversar con el grupo" : "¿Qué nos dicen las respuestas?"}</h1><p className="sub">Una exploración interactiva basada en respuestas de la clase.</p></div><div className="header-actions"><select aria-label="Filtrar por experiencia" value={filter} onChange={(e) => setFilter(e.target.value)}><option>Todos</option>{experiences.map((x) => <option key={x}>{x}</option>)}</select><button className="outline" onClick={() => setFilter("Todos")}>Reiniciar</button></div></header>
      ...
      {mode === "class" ? <ClassMode overall={overall} strongest={strongest.dimension.name} opportunity={opportunity.dimension.name} reveal={reveal} setReveal={setReveal} story={story} addFinding={addFinding} /> : <>
        ...
        {tab === "brechas" && scores.some((item) => item.dimension.id === "strategy") && scores.some((item) => item.dimension.id === "adoption") && scores.some((item) => item.dimension.id === "governance") ? <Gaps strategy={scores.find((x) => x.dimension.id === "strategy")!.value} execution={mean([scores.find((x) => x.dimension.id === "adoption")!.value, scores.find((x) => x.dimension.id === "governance")!.value])} count={filtered.length} addFinding={addFinding} /> : <section className="card"><p className="eyebrow">BRECHAS Y CONTRADICCIONES</p><h2>Faltan dimensiones para calcular esta tensión.</h2><p>Importá columnas de estrategia, gobierno y adopción para habilitar este análisis.</p></section>}
...
function Questions({ scores, count }: { scores: { dimension: Dimension; value: number }[]; count: number }) { const [selected, setSelected] = useState(scores[0]?.dimension.id ?? ""); const dimension = scores.find((score) => score.dimension.id === selected)?.dimension ?? scores[0].dimension; const value = scores.find((score) => score.dimension.id === dimension.id)?.value ?? 0; return <section className="card"><p className="eyebrow">EXPLORADOR DE PREGUNTAS</p><h2>Distribución y evidencia</h2><div className="chips">{scores.map(({ dimension: item }) => <button className={selected === item.id ? "selected" : ""} key={item.id} onClick={() => setSelected(item.id)}>{item.name}</button>)}</div>{dimension.questions.map((question, i) => { const a = Math.max(8, Math.round(12 + value * 4 + i * 2)); const b = 100 - a - 23; return <article className="question" key={question}><div><b>{question}</b><small>{dimension.name} · {count} respuestas · 2 omitidas</small></div><div className="distribution"><i style={{ width: `${a}%` }} /><i style={{ width: "23%" }} /><i style={{ width: `${b}%` }} /></div><span>{(value + (i - 1) * .15).toFixed(1)} / 5</span></article>; })}</section>; }
